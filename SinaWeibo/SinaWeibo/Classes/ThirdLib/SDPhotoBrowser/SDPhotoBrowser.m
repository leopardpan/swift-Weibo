...
- (void)setupImageOfImageViewForIndex:(NSInteger)index imageView:(SDBrowserImageView*)imageView andUrl:(NSArray*)url
{
//        SDBrowserImageView *imageView = _scrollView.subviews[index];
...
//    NSLog(@"%@",url[index]);
    ...
    if (imageView.hasLoadedImage) return;
    if (url[index]) {
        [imageView setImageWithURL:url[index] placeholderImage:nil];
    } else {
        imageView.image = nil;
    }
...
//    NSLog(@"%d",index);
    ...
    [self setupImageOfImageViewForIndex:index imageView:self.imageView andUrl:self.url];
}
